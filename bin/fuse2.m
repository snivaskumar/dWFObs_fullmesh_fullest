function [zf, Zf, xf] = fuse2(z1,z2,Z1,Z2,x1,x2,type,fusion_weight,constant);
...
if strcmp(type,'CIN')
    Zf      = inv(inv(X_a) + inv(X_b));
    zf      = Zf*(inv(X_a)*x_a + inv(X_b)*x_b);
    0;
elseif strcmp(type,'CI2')
    ...
    if strcmp(fusion_weight,'OPTIMAL')
        f       = @(w) trace( inv(w*inv(ZA) + (1-w)*inv(ZA)) ); % arg (min -f) = arg (max f)
        omega   = fminbnd(f,0,1,optimset('Display','off'));
    else
        omega   = constant;
    end
    Zf      = inv(omega*inv(ZA) + (1-omega)*inv(ZA));
    zf      = Zf*(omega*inv(ZA)*x_a + (1-omega)*inv(ZB)*x_b);
    1;
...
elseif strcmp(type,'ICI')
    ...
    ZB      = X_b;
    if strcmp(fusion_weight,'OPTIMAL')
        ff      = @(w) trace( (inv(ZA) + inv(ZB) - inv(w*ZA + (1-w)*ZB))); % min -f = max f
        omega   = fminbnd(ff,0,1,optimset('Display','off'));
    else
        omega   = constant;
    end
    Xij     = omega*ZA + (1-omega)*ZB;
    Zf      = inv(inv(X_a) + inv(X_b) - inv(Xij));

    K = Zf*(inv(X_a) - omega*inv(Xij));
    L = Zf*(inv(X_b) - (1 - omega)*inv(Xij));
    zf = K*x_a + L*x_b;
    3;
end
xf  = x_tmp;
