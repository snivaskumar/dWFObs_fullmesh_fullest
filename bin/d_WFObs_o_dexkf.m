function [Wp,sol_out,strucObs] = d_WFObs_o_dexkf(strucObs,Wp,sys_in,sol_in,options)
...
if sol_in.k == 1
    if strcmp(domainShape,'SQUARE')
        disp('Shape of the Domain: Square')
    elseif strcmp(domainShape,'CIRCLE')
        disp('Shape of the Domain: Circle')
    else
        disp('Selected domain shape is not available')
    end
...
if sol_in.k == 1
    ...
    k = 1;
    for iii = Wp.turbine.ID
        turbLocArray(k,:) = [Wp.turbine.Crx(iii),Wp.turbine.Cry(iii)];
        k = k + 1;
    end
...
% tic
[xkk Pkk] = d_distributed_linear( strucObs,x,d,p,l,n, F,D,E,G,H,Q,R, y, xkk1,xk1k1,Sk1k1, x_est,x_unest, P_unest, type,typeCZ );
% toc
